...
export async function generatePass () {
    ...
    await template.images.add("icon", process.cwd() + "/lib/assets/zipLogo.png");
    await template.images.add("strip", process.cwd() + "/lib/assets/banner.png", "3x");

    await addAsset(template, "logo", process.cwd() + "/lib/assets/hackathonLogo.png", 160);

    const bannerImageBuffer = await fs.readFile(process.cwd() + "/lib/assets/banner.png");
